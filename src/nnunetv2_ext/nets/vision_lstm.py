...
from nnunetv2_ext.nets.vision_lstm_util import interpolate_sincos, to_ntuple, VitPatchEmbed, VitPosEmbed2d, DropPath
...
class VIL3DPatchEncoder(nn.Module):
    def __init__(
            self,
        ...
        # initialize patch_embed
        self.patch_embed1 = VitPatchEmbed(
            dim=embed_dims[0],
            stride=8,
            num_channels=self.input_shape[0],
            resolution=self.input_shape[1:],
        ...
        self.patch_embed2 = VitPatchEmbed(
            dim=embed_dims[1],
            stride=stride,
            num_channels=self.embed_dims[0],
            resolution=(self.input_shape[1]//2,)*3,
            patch_size=self.patch_size,
        )
        self.patch_embed3 = VitPatchEmbed(
            dim=embed_dims[2],
            stride=stride,
            num_channels=self.embed_dims[1],
            resolution=(self.input_shape[1]//4,)*3,
            patch_size=self.patch_size,
        )
        self.patch_embed4 = VitPatchEmbed(
            dim=embed_dims[3],
            stride=stride,
            num_channels=self.embed_dims[2],
            resolution=(self.input_shape[1]//8,)*3,
            patch_size=self.patch_size,
        )
        ...
        depth_all = 0
        for depth_single in self.depths:
            depth_all += depth_single
        ...
        self.norm3 = LayerNorm(embed_dims[2])
        ...
        self.norm4 = LayerNorm(embed_dims[3])
...
class VIL3DPatchEncoder3dconv(nn.Module):
    def __init__(
            self,
            embed_dims=[48, 96, 192, 384],
            input_shape=(4, 128, 128, 128),
        ...
        self.proj_conv3d_1 = nn.Conv3d(self.input_shape[0], embed_dim[0], kernel_size=patch_size, stride=stride,
                              padding=patch_size // 2, bias=False)
        self.proj_norm_1 = nn.BatchNorm3d(embed_dim)
